...
class _CRG(Module):
    def __init__(self, platform, sys_clk_freq, with_ethernet, with_vga):
        ...
        # Overclock switch: SW3 selects between sys_clk_freq (normal) and
        # 1.5x sys_clk_freq (overclocked) via a glitchless BUFGMUX.
        # NOTE: UART baud timing is derived from sys_clk_freq at build time,
        # so the serial console will garble while overclocked -- the LED
        # chase software demo is the reliable indicator that the CPU is
        # actually running faster.
        self.clock_domains.cd_sys_norm = ClockDomain(reset_less=True)
        self.clock_domains.cd_sys_fast = ClockDomain(reset_less=True)
        pll.create_clkout(self.cd_sys_norm, sys_clk_freq,       buf=None)
        pll.create_clkout(self.cd_sys_fast, 1.5*sys_clk_freq,   buf=None)

        # sw3 is active-low (pulled high, pulled to 0V when pressed), so
        # invert it: unpressed (1) -> normal clock, pressed (0) -> fast clock.
        # It's a raw async board input, so it must be synchronized before it
        # can safely drive a BUFGMUX select -- an unsynchronized select edge
        # arriving while the PLL is still locking at reset is a known glitch
        # source (Xilinx's own guidance: always synchronize BUFGMUX select).
        sw3 = platform.request("sw3")
        self.sw3 = sw3  # exposed so BaseSoC can also give software a readback of this signal
        overclock_sel_raw = Signal()
        self.comb += overclock_sel_raw.eq(~sw3)
        overclock_sel = Signal()
        self.specials += MultiReg(overclock_sel_raw, overclock_sel, odomain="sys_norm")
        self.specials += Instance("BUFGMUX",
            i_S  = overclock_sel,
            i_I0 = self.cd_sys_norm.clk,
            i_I1 = self.cd_sys_fast.clk,
            o_O  = self.cd_sys.clk,
        )
...
class BaseSoC(SoCCore):
    def __init__(self, toolchain="vivado", sys_clk_freq=int(100e6),
        ...
        # Software-readable copy of the overclock select button, so firmware can
        # know when to reprogram the UART's dynamic baudrate tuning word.
        self.submodules.overclock_btn = GPIOIn(self.crg.sw3)
