...
// Tuning word = (baudrate / clk_freq) * 2^32, recomputed for each clock speed
// the CRG's BUFGMUX can select, so the UART's actual bit-timing stays correct
// regardless of which one is active. Requires building with
// --uart-with-dynamic-baudrate.
#define UART_BAUDRATE            115200
#define TUNING_WORD_NORMAL       4947802   // 115200 baud @ 100MHz
#define TUNING_WORD_OVERCLOCKED  3298534   // 115200 baud @ 150MHz

// overclock_btn is active-low (sw3 pulled high, pulled to 0V when pressed).
static void sync_uart_baud_to_clock(void)
{
	if (overclock_btn_in_read())
		uart_phy_tuning_word_write(TUNING_WORD_NORMAL);
	else
		uart_phy_tuning_word_write(TUNING_WORD_OVERCLOCKED);
}
